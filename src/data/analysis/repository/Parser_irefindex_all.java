...
//				"<http://dbmi-icode-01.dbmi.pitt.edu/linkedSPLs/resource/structuredProductLabelMetadata/bb81de7b-c542-495b-9278-bb8bbd6ff199>");
	...
	public static void getType(String input) throws IOException{
		...
		while((line=br.readLine())!=null){
				...
				while (nxp.hasNext()) {
					...
					if(p.contains("<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>")) {
						String standard_s=s.substring(0,s.lastIndexOf(":"));
						String standard_o=o.substring(0,o.lastIndexOf(":"));
						set.add(standard_s+" "+p+" "+standard_o);
					}
				}
		}
		br.close();
...
//		external: -> <http://bio2rdf.org/omim_resource:ffe2638260c8f14cc95cb8023edc3d0f> <http://bio2rdf.org/omim_vocabulary:x-umls> <http://bio2rdf.org/umls:C1855201>
		...
		while((line=br.readLine())!=null){
			if(!line.contains("\"")){
				...
				while (nxp.hasNext()) {
					...
					if(!exclude.contains(p)&&s.contains("<http://")&&o.contains("<http://")) {
						String standard_s=s.substring(0,s.lastIndexOf("/"));
						String standard_o=o.substring(0,o.lastIndexOf("/"));
						triples.add(line);
						if(p.contains(":x-")) {
	...
	public static void getsubject_advance(String input, String output) throws IOException{
		BufferedReader br = new BufferedReader(new FileReader(new File(input)));
		String line=null;
		HashMap<String, String> typesHashMap =new HashMap<>();
		HashMap<String, HashMap<String, HashSet<String>>> dataMap =new HashMap<>();
			...
			if(!line.contains("\"")){
				...
				while (nxp.hasNext()) {
					...
					if(p.equals("<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>")) {
						typesHashMap.put(s, o);
					}
					if(p.equals("<http://bio2rdf.org/irefindex_vocabulary:interactor_a>")) {
						subjects.add(s);
						
						if(dataMap.containsKey(s)) {
							if(dataMap.get(s).containsKey("a")) {
								dataMap.get(s).get("a").add(o);
							}else {
								HashSet<String> set=new HashSet<>();
								set.add(o);
								dataMap.get(s).put("a", set);
							}
						}else {
							HashSet<String> set=new HashSet<>();
							set.add(o);
					...
					if(p.equals("<http://bio2rdf.org/irefindex_vocabulary:interactor_b>")) {
						...
						if(dataMap.containsKey(s)) {
							if(dataMap.get(s).containsKey("b")) {
								dataMap.get(s).get("b").add(o);
							}else {
								HashSet<String> set=new HashSet<>();
								set.add(o);
								dataMap.get(s).put("b", set);
							}
						}else {
							HashSet<String> set=new HashSet<>();
							set.add(o);
		...
		for(String subject:subjects) {
			if(typesHashMap.containsKey(subject)) {
				typeSet.add(typesHashMap.get(subject));
			}
		}
		...
		for(Entry<String, HashMap<String, HashSet<String>>> entry_1:dataMap.entrySet()) {
			for(Entry<String, HashSet<String>> entry_2:entry_1.getValue().entrySet()) {
				bw.write(typesHashMap.get(entry_1.getKey())+" ->"+entry_1+"\n");
			}
		}
		bw.flush();
		bw.close();
	...
	public static void getinstance(String input, String instance) throws IOException{
		BufferedReader br = new BufferedReader(new FileReader(new File(input)));
		String line=null;
		while((line=br.readLine())!=null){
				InputStream inputStream = new ByteArrayInputStream(line.getBytes());
