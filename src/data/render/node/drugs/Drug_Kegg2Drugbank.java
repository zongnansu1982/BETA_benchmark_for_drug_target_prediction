...
public class Drug_Kegg2Drugbank {
	...
	public static HashMap<String,HashSet<String>> getKegg(String input) throws IOException{
		...
		while((line=br.readLine())!=null){
			if(!line.contains("\"")){
				...
				while (nxp.hasNext()) {
					...
					if(p.equals("<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>")
							&o.equals("<http://bio2rdf.org/kegg_vocabulary:drug>")){
						drugs.add(s);
					}
					
					if(p.equals("<http://bio2rdf.org/kegg_vocabulary:x-drugbank>")
		...
		System.out.println("Kegg drugbank: "+todrugBank.size());
		System.out.println("Kegg drugs: "+drugs.size());
		br.close();
		return todrugBank;
	}
	...
	public static void writeMapping(HashMap<String,HashSet<String>> drugbank_1,HashMap<String,HashSet<String>> drugbank_2,String outfile) throws IOException{
		...
		System.out.println("@@@ Kegg2Drugbank mapping: "+mappings.size());
		System.out.println("@@@ mapped kegg: "+nodes1.size());
