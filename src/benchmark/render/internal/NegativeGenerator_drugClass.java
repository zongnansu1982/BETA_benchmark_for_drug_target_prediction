...
public class NegativeGenerator_drugClass {
...
public NegativeSetBean generate(long seed, Double ratio,String type, HashSet<String> similarDrugs,HashSet<String> positive_train, HashSet<String> positive_test ){
		 ...
		 */
			...
			if(type.equals("trt")) {
				...
				for(String triple:positive_test) {
					...
					while (nxp.hasNext()) {
						...
						String o = quard[2].toString().trim();
						if (p.equals("<http://bio2rdf.org/drugbank_vocabulary:drug-target>")) {
							if(local_targetSet_1.contains(o)) {
								local_targetSet_1.remove(o);								
							}
						} 
					}	
				}
			...
			if(type.equals("te")) {
				...
				local_drugSet_1.removeAll(similarDrugs);
			}
	...
	public NegativeGenerator_drugClass (HashSet<String> positive_train, HashSet<String> positive_test) throws IOException{
